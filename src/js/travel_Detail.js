...
function str_dea_updatenum(id, num) {
    ...
    var sql = 'UPDATE travel SET num =' + num + ' WHERE id =' + id;
...
function str_dea_search() {
    $('.act_search').on('click', function() {
        ...
        $(this).parent().attr('href', 'travel.html?title=' + search_val);
    })
}
str_dea_search();
...
function str_dea_each_item() {
    ...
    getPHP("src/php/query.php?sql=" + sql).then(function(php) {
        ...
        for (var i = 0; i < 3; i++) {
            str += ` 
            <li><a href="travel_detail.html?id=${obj[i].id}" rel='${i}'>${obj[i].title}</a></li>
            `;
        }
        $('.echo_item').html(str);
        //修改阅读量
...
function str_detail_info() {
    ...
    var sql = 'SELECT * FROM travel where id=' + id;
    getPHP("src/php/query.php?sql=" + sql).then(function(php) {
        var obj = JSON.parse(php);
        console.log(obj)
